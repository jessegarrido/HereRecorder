    ...
    public class SettingsService : ISettingsService
    {
		...
		public async Task<bool> GetDownmix()
		{
			return await System.Text.Json.JsonSerializer.DeserializeAsync<bool>
						...
						(await _httpClient.GetStreamAsync($"api/getmono"), new JsonSerializerOptions() { PropertyNameCaseInsensitive = true });
		}
        ...
        public async Task<bool> GetNormalizeSplitChannels()
        {
            return await System.Text.Json.JsonSerializer.DeserializeAsync<bool>
            // return OK(await _httpClient.GetStreamAsync($"api/getnormalized"))
            (await _httpClient.GetStreamAsync($"api/getnormalizesplitchannels"), new JsonSerializerOptions() { PropertyNameCaseInsensitive = true });
        }
        public async Task<IActionResult> SetNormalizeSplitChannels(bool splitChannels)
        {
            return await _httpClient.GetAsync($"api/setnormalizesplitchannels/{splitChannels}") as IActionResult;
        }
